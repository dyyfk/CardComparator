...
export default {
  name: "numerialDistribution",
  props: ["dis", "v1", "v2", "index"],
  data() {
    return {
      svg: null,
      bar: null
  ...
  mounted() {
    // console.log(this.$refs.distribution);
    // console.log(this.$);
  },
  methods: {
    createDistribution() {
      ...
      // console.log(this.$refs["distribution" + this.index]);

      var values = this.dis.map(e => numeral(e)._value); // clean the distribution
      ...
      var data = d3.histogram().thresholds(x.ticks(10))(values);
